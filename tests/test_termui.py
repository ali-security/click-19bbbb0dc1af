...
def test_editor_path_normalization(editor_cmd, filenames, expected_args):
    """The editor command is split into an argv list and passed to ``Popen``
    without a shell, so neither it nor the filenames are interpreted.
    """
...
def test_editor_env_var_not_shell_interpreted(monkeypatch, env_key):
    """An editor taken from ``VISUAL``/``EDITOR`` is not run through a shell,
    so it cannot be used to inject commands.
    """
...
def test_editor_env_var_no_command_injection(monkeypatch, tmp_path):
    """An ``EDITOR`` value carrying shell metacharacters must not execute the
    injected command.
    """
...
@pytest.mark.skipif(WIN, reason='Windows filenames cannot contain " or ;.')
def test_edit_filename_no_command_injection(monkeypatch, tmp_path):
    """A filename carrying shell metacharacters must not break out of the
    editor invocation and execute the injected command.
    """
    monkeypatch.chdir(tmp_path)
    marker = tmp_path / "pwned.txt"
    target = tmp_path / 'a"; touch pwned.txt; echo "b.txt'
    target.write_text("hello")

    click.edit(filename=str(target), editor="cat")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Shell metacharacters must survive as plain argv items.
        pytest.param(
            "less -R ; touch pwned",
            ["less", "-R", ";", "touch", "pwned"],
...
def test_pager_env_passed_to_popen_as_argv(monkeypatch, pager_env, expected_parts):
    ...
    See issue #1026, PR #1477, PR #1543 and PR #2775.
    """
    monkeypatch.setattr(click._termui_impl, "WIN", False)
    monkeypatch.setattr(click._termui_impl, "isatty", lambda x: True)
    # The pager is resolved on PATH before it is run, which no test can rely
    # on for these made up commands.
    ...
    mock_popen.assert_called_once()
    # ``_pipepager`` invokes the resolved absolute path of the command.
    expected_argv = [str(Path(expected_parts[0]).resolve()), *expected_parts[1:]]
    assert mock_popen.call_args[0][0] == expected_argv
...
def test_pager_env_split_into_argv(monkeypatch, win):
    """``pager()`` hands the ``PAGER`` value to its helpers as a POSIX-split
    argv list, never as a shell command string.
    """
    recorded = []
    ...
    monkeypatch.setattr(click._termui_impl, "_tempfilepager", record)
    monkeypatch.setattr(click._termui_impl, "isatty", lambda x: True)
    monkeypatch.setitem(os.environ, "PAGER", '"my pager" -R ; touch pwned')

    click.echo_via_pager("hello")

    assert recorded == [["my pager", "-R", ";", "touch", "pwned"]]


@pytest.mark.skipif(WIN, reason="Uses POSIX shell syntax and executable bits.")
def test_pager_path_not_shell_interpreted(monkeypatch, tmp_path):
    """``_pipepager`` runs the pager as ``argv``, so metacharacters in the
    resolved command path can't be used to run a second command.
    """
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    marker = tmp_path / "pwned"
    # A shell would read this name as "run <bin_dir>/pager, then create the
    # file pwned"; exec() just runs the file that carries the odd name.
    pager_name = "pager;>pwned"
    pager_path = bin_dir / pager_name
    pager_path.write_text("#!/bin/sh\ncat > /dev/null\n")
    pager_path.chmod(0o755)

    monkeypatch.chdir(tmp_path)
    monkeypatch.setitem(
        os.environ, "PATH", str(bin_dir) + os.pathsep + os.environ["PATH"]
    )
    monkeypatch.setitem(os.environ, "PAGER", pager_name)
    monkeypatch.setattr(click._termui_impl, "isatty", lambda x: True)
